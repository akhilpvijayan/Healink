...
export class PostCommentsDialogComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<PostCommentsDialogComponent>,
  ...
  ngOnInit(): void {
    this.postservice.getComments(this.skip, this.take, this.data?.postDetails?.postId).subscribe((res: any) => {
      if(res[0]){
  ...
  addComment() {
    this.signalRService.postComment(this.userService.getUserId(), this.data?.postDetails?.postId, this.data?.postDetails?.userId, this.commentContent, false, 0);
    this.commentContent = '';
  }

  deleteComment(commentId: number) {
    this.signalRService.postComment(this.userService.getUserId(), this.data?.postDetails?.postId, this.data?.postDetails?.userId, this.commentContent, true, commentId);
  }
  ...
  saveComment(comment: any, updatedContent: string): void {
    ...
    this.signalRService.postComment(this.userService.getUserId(), this.data?.postDetails?.postId, this.data?.postDetails?.userId, updatedContent, false, comment.commentId);
  }
